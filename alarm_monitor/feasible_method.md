...
The current monitor has moved past the original cheap-label sketch and uses
tree-derived soft labels:

1. **Implemented baseline**: train on mixed-root tree data, using each
   prefix's node-level subtree success rate `target_y` as a soft probability.
2. **Pairwise structure**: add sibling Bradley-Terry comparisons so the model
   learns which continuation looks more promising under a shared parent.
3. **Pure-root evaluation**: tune and report alarms only on pure-root
   validation/test files, where `pure0` means a failing leaf path and `pure1`
   means a successful leaf path.

The older Phase 1 idea, using only eventual success of the original run as a
hard per-prefix label, remains a possible cheap ablation but is no longer the
main path.

---

## Downstream Use of `steps.csv`: Current Baseline

The first monitor now lives in
[`prefix_alarm_monitor.ipynb`](prefix_alarm_monitor.ipynb) and
[`prefix_alarm_monitor_lib.py`](prefix_alarm_monitor_lib.py). It consumes the
`steps.csv` files described above and trains a compact prefix-level model:

- features are computed over a trailing 64-step root-to-node window,
- training uses mixed-root files only,
- validation/test use pure-root files only,
- the raw score is a linear Bradley-Terry model trained on sibling
  comparisons, with a small row-level soft-label anchor,
- a logistic calibrator maps raw score to `p_success`,
- alarm thresholds are tuned at the leaf-trajectory level, not the individual
  row level.

The current default rule is `calibrated_leaf_low_fp`:

\[
\text{alarm}_i = \mathbf{1}[p_{\text{success},i} < \theta_p].
\]

The threshold \(\theta_p\) is chosen on validation to minimize false alarms on
successful pure leaf paths subject to a target recall on failing pure leaf
paths:

\[
\text{pure0 leaf recall} \ge 0.85.
\]

Two comparison rules are also reported:

- `cusum_leaf_low_fp`: a sequential CUSUM over
  \(1 - p_{\text{success}}\), tuned with the same leaf-level objective.
- `gated_leaf_low_fp`: the calibrated alarm confirmed by either a low
  lower-confidence-bound score or a sharp raw-score drop from the parent.

Latest full-feature test metrics:
...
The monitor still does not evaluate interventions online. These metrics answer
whether an offline rule would have warned on a failing leaf path before the
final step, and how often it would falsely warn on a successful leaf path.

The notebook also includes a CUSUM backtracking diagnostic: after a CUSUM
alarm fires, it traces backward to the shortest recent contribution window
whose cumulative increments explain the threshold crossing. In the latest
run, CUSUM alarms can be explained by roughly the previous 32-35 real steps
on average, but this is post-hoc explanation rather than an online early-stop
claim.
...
Current baseline: linear Bradley-Terry prefix score plus logistic calibration
and three leaf-level monitor rules. See
[`MONITOR_TRAINING.md`](MONITOR_TRAINING.md) for the model, formulas, metrics,
and latest numbers.

### Stage 4 — intervention evaluation
Still deferred. The current work evaluates alarms offline; it does not yet
rerun the agent with early stopping, replan, or branch-pruning interventions.
...
The current first-pass consumer of those CSVs is the prefix alarm monitor:
a linear calibrated success model plus leaf-level alarm rules. Intervention
